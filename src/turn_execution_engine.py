...
from src.invariants import (
    ...
    NON_SEMANTIC_EVENT_FIELDS,
    canonical_events_hash as invariant_canonical_events_hash,
    run_invariants,
)
from src.replay_utils import (
    canonical_json as replay_canonical_json,
    replay_state as replay_state_impl,
    state_hash as replay_state_hash,
)
from src.mpp_stage_pipeline import MPPStageError, enforce_mpp_stages
...
class EnginePaths:
    ...
    mpp_stage_pipeline: Path
    mpp_stage_policy_schema: Path
    mpp_stage_artifact_schema: Path
...
def _canonical_json(payload: Any) -> str:
    return replay_canonical_json(payload)


def _state_hash(state: dict[str, Any]) -> str:
    return replay_state_hash(state)
...
def canonical_events_hash(events: list[dict[str, Any]]) -> str:
    return invariant_canonical_events_hash(events)
    ...
    try:
        ...
        if chain_path.exists():
            ...
            for i in range(1, len(lines)):
                prior_hash = lines[i - 1].get(
                    "current_hash", lines[i - 1].get("entry_hash")
                )
                if lines[i].get("prev_hash") != prior_hash:
...
def replay_state(events: list[dict[str, Any]]) -> dict[str, Any]:
    return replay_state_impl(events)
...
def _load_control_files(paths: EnginePaths) -> None:
    ...
    _read_text(paths.mpp_stage_pipeline)
    _read_text(paths.mpp_stage_policy_schema)
    _read_text(paths.mpp_stage_artifact_schema)
...
def _enforce_invariant_coverage_map(paths: EnginePaths) -> None:
    coverage_path = paths.root / "INVARIANT_COVERAGE_MAP.md"
    text = _read_text(coverage_path)
    rows = []
    for line in text.splitlines():
        if not line.startswith("|") or "Critical property" in line or "---" in line:
            continue
    ...
    if not rows:
        raise EngineError("Invariant coverage map is missing property rows")
    properties = [prop for prop, _ in rows]
    if len(properties) != len(set(properties)):
        raise EngineError("Invariant coverage map has duplicate property mappings")
    invariants = [inv for _, inv in rows]
    if len(invariants) != len(set(invariants)):
        raise EngineError("Invariant coverage map has duplicate invariant mappings")
    known = {inv.name for inv in INVARIANT_REGISTRY}
    unknown = [name for name in invariants if name not in known]
    if unknown:
        raise EngineError(
            f"Invariant coverage map references unknown invariants: {unknown}"
        )
    if set(invariants) != known:
        missing = sorted(known.difference(invariants))
        extra = sorted(set(invariants).difference(known))
        raise EngineError(
            f"Invariant coverage map mismatch: missing={missing}, extra={extra}"
        )


def _enforce_immutable_config(paths: EnginePaths) -> None:
    immutable_path = paths.root / "IMMUTABLE_CONFIG.json"
    if not immutable_path.exists():
        return
    payload = _read_json(immutable_path)
    expected_version = payload.get("invariant_registry_version")
    if expected_version != INVARIANT_REGISTRY_VERSION:
        raise EngineError("Immutable config mismatch: invariant registry version")
    expected_non_semantic = payload.get("non_semantic_event_fields", [])
    if sorted(expected_non_semantic) != sorted(NON_SEMANTIC_EVENT_FIELDS):
        raise EngineError("Immutable config mismatch: canonicalization rules")
    expected_coverage_hash = payload.get("coverage_map_sha256")
    if expected_coverage_hash:
        coverage_hash = hashlib.sha256(
            _read_text(paths.root / "INVARIANT_COVERAGE_MAP.md").encode("utf-8")
        ).hexdigest()
        if coverage_hash != expected_coverage_hash:
            raise EngineError("Immutable config mismatch: coverage map hash")


def _run_runtime_integrity_invariants(
    *,
    proofs: list[dict[str, Any]],
    events: list[dict[str, Any]],
    runtime_state_invalid: bool = False,
    mpp_compliance: dict[str, Any] | None = None,
    root_path: Path | None = None,
) -> None:
    snapshot_hash = proof_registry_snapshot_hash(proofs)
    version = proofs[-1].get("invariant_registry_version", "") if proofs else ""
    latest = proofs[-1] if proofs else {}
    latest_mpp_hash = latest.get("mpp_hash", "") if latest else ""
    latest_bts_hash = latest.get("bts_hash", "") if latest else ""
    latest_optimality_hash = latest.get("optimality_hash", "") if latest else ""
    try:
        report = run_invariants(
            {
                ...
                "invariant_registry_version": version,
                "expected_manifest_hash": "",
                "actual_manifest_hash": "",
                "mpp_compliance": mpp_compliance,
                "mpp_task_id": latest.get("task_id")
                if latest and latest_mpp_hash
                else None,
                "proof_mpp_hash": latest_mpp_hash,
                "bts_task_id": latest.get("task_id")
                if latest and latest_bts_hash
                else None,
                "proof_bts_hash": latest_bts_hash,
                "optimality_task_id": latest.get("task_id")
                if latest and latest_optimality_hash
                else None,
                "proof_optimality_hash": latest_optimality_hash,
        ...
        raise EngineError("Runtime invariant contract failure: " + str(exc)) from exc
    if not report["passed"]:
        raise EngineError(f"Runtime invariant failure: {report['failures'][0]['name']}")


def _runtime_tamper_check(paths: EnginePaths, *, fail_closed: bool = True) -> None:
    snapshot_path = paths.root / "proof_registry_snapshot.sha256"
    if not snapshot_path.exists():
        return
    proofs = load_registry_entries_resilient(paths.root / "PROOF_REGISTRY.jsonl")
    expected = snapshot_path.read_text(encoding="utf-8").strip()
    actual = proof_registry_snapshot_hash(proofs)
    if expected != actual and fail_closed:
        raise EngineError("Runtime tamper check failed: proof snapshot mismatch")
...
def _execute_once_internal(
    ...
    _enforce_invariant_coverage_map(paths)
    _enforce_immutable_config(paths)
    ...
    if bounded_stage:
        ...
        params = bounded_stage.get("params", {})
        if bool(params.get("mpp_required", False)):
            mpp_task_id = str(params.get("mpp_task_id", "")).strip()
            if not mpp_task_id:
                raise EngineError("MPP enforcement requires non-empty mpp_task_id")
            try:
                mpp_compliance = enforce_mpp_stages(
                    root=paths.root,
                    task_id=mpp_task_id,
                    policy=params.get("mpp_policy"),
                )
            except MPPStageError as exc:
                raise EngineError("MPP stage enforcement failed: " + str(exc)) from exc
            try:
                mpp_hash = canonical_mpp_hash(paths.root, mpp_task_id)
            except MPPCanonicalError as exc:
                raise EngineError("MPP hash calculation failed: " + str(exc)) from exc
            if not mpp_hash:
                raise EngineError("MPP hash calculation failed: empty hash")
            try:
                bts_hash = canonical_bts_hash(paths.root, mpp_task_id)
            except BTSCanonicalError as exc:
                raise EngineError("BTS hash calculation failed: " + str(exc)) from exc
            if not bts_hash:
                raise EngineError("BTS hash calculation failed: empty hash")
            try:
                optimality_hash = canonical_optimality_hash(paths.root, mpp_task_id)
            except OptimalityError as exc:
                raise EngineError(
                    "Optimality hash calculation failed: " + str(exc)
                ) from exc
            if not optimality_hash:
                raise EngineError("Optimality hash calculation failed: empty hash")
    ...
    if candidate:
            ...
            va = run_validation_pipeline(
                {
                    ...
                    "task_id": mpp_task_id or "stage2-turn-execution",
                    ...
                    "mpp_hash": mpp_hash,
                    "bts_task_id": mpp_task_id,
                    "bts_hash": bts_hash,
                    "optimality_task_id": mpp_task_id,
                    "optimality_hash": optimality_hash,
        ...
        if mpp_task_id:
            bts_counterfactual = {
                "chosen_option": mpp_compliance.get("policy", {}).get("task_id")
                if mpp_compliance
                else None,
                "rejected_options": [],
                "predicted_scores": {},
                "actual_outcome": {
                    "executed_stage_id": candidate["payload"]["stage_id"],
                    "result": "PASS",
                },
                "regret_score": 0.0,
            }
            bts_dir = paths.root / "bts_artifacts" / mpp_task_id
            bts_dir.mkdir(parents=True, exist_ok=True)
            _atomic_write_text(
                bts_dir / "post_execution_counterfactual.json",
                json.dumps(bts_counterfactual, sort_keys=True, indent=2) + "\n",
            )
        validation_artifacts["PROOF_REGISTRY_ENTRY"] = json.dumps(
            va.proof_registry_entry, sort_keys=True
        )
        _run_runtime_integrity_invariants(
            proofs=load_registry_entries_resilient(paths.root / "PROOF_REGISTRY.jsonl"),
    ...
    try:
        ...
        if candidate:
            ...
            _run_runtime_integrity_invariants(
                proofs=load_registry_entries_resilient(
                    paths.root / "PROOF_REGISTRY.jsonl"
                ),
                events=_load_events(paths.events),
                runtime_state_invalid=False,
...
def execute_with_recovery(
    ...
    *,
    ...
    _runtime_tamper_check(paths, fail_closed=True)
