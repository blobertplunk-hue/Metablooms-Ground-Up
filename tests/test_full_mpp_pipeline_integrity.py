...
def test_full_mpp_pipeline_integrity(tmp_path: Path) -> None:
    ...
    expected_state_hash = hashlib.sha256(
        json.dumps(replayed, sort_keys=True, separators=(",", ":")).encode("utf-8")
    ).hexdigest()
    stage_executed = [e for e in all_events if e["type"] == "STAGE_EXECUTED"][-1]
    assert stage_executed["state_hash_after"] == expected_state_hash
